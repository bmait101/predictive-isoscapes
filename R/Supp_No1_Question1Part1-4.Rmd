---
title: 'Maitland Comprehensive Exam::Supplement No.1 - Question 1, Parts 1-4'
...
# Objectives
1. Using the rKIN package, create an isospace for prey in one study site. Site must contain ≥ 3 prey species. 

2. Model the feeding behaivor of a predator species by adapting the provided MATLAB code to R. These are called broken stick models and were first proposed by McArthur (1957, 1961) to describe the packing of species by niche partitioning (see illustration below). Nonetheless, they have been used to model dietary contributions (Flaherty and Ben-David 2010). 
Use model 4 (one habitat, multiple prey): In the attached MatLab code the "kestrels" are randomly foraging on each prey type drawing from a normal distribution of prey isotopic signatures. You should be using the results from rKIN such that the predator consumes prey according to their real distribution. For example, if your predator consumes 20% of prey A, it should be consuming 10% of that prey within the 50% contour of the KUD.
...
## 1. Create isospace for prey using rKIN
This will model prey isotopic distributions at different contour interval to be used in subsequent foraging modeling of a predator
```{r}
### Load data
isodat <- as.tibble(read.csv("data_raw_rKIN.csv", header = TRUE))
isodat1  <- isodat %>% filter(SpecCode.new != "BNT") %>% droplevels() 

### Create matrix d.f. for analyses; must put into DF not tibble
exo <- as.data.frame(isodat1[,c("d13C","d15N","SpecCode.new","SiteID")])
...
# randomly sampling the kernal estimates from rKIN, then feed into forage model
# For simplicity, only using 95% contour interval for this analysis
...
plotKIN(tkin, title="Prey Kernel UD for LR05 (Laramie R @ Laramie)", 
        xlab = expression({delta}^13*C~'\u2030'), 
        ylab = expression({delta}^15*N~'\u2030'))

### 2. For each prey kernal, sample randomly 100 times to generate observed 
#      distriution prey isotopic distributions
...
### 3. Extract x,y coordinates from list into new df
df.samples <- 
  data.frame(rbind(
...
#### 4. Compile results into a new df to feed into foraging model - pun intended :)
...
df.samples$Taxa <- Taxa  # add taxa column to df
df.samples <- df.samples %>% select(Taxa, m.d13C, sd.d13C, m.d15N, sd.d15N)  # rearrange
order <- c("Decapoda","Minnow","Baetidae","Hydropsychidae","Isopoda")
(prey <- as.data.frame(df.samples %>% slice(match(order, Taxa))) )  # print results
```

## 2. Model the feeding behaivor of a predator
1. Brown Trout eating 5 prey items (fish and invetebrates)
2. Model will choose the percentage of diet made up of the prey items for 25 Brown Trout
3. Model will choose stable isotope values from KUD ditributions and report final niche for 25 Brown Trout
```{r}
#===============================================================================
# --- Simulating foraging behaivor  ---- 
#===============================================================================
# NOTE: code adapted from Flaherty & Ben-David (2010)
# NOTE: prey df has been created in previous code chunk
...
### 2. Loop through each virtual predator, calculate diet propotions, assaign C & N values
for (i in 1:N) {
  ...
  # Store diet proportion results in a outdf; calcualte C and N values for each prey and 
...
### 3. Get average diet proportions for all 25 simulated Brown Trout; print it
(diet.p.sim <- data.frame(Taxa = "Simulated_BNT", 
...
## 3. Compare simulated predators to the observed isotopic signature for that predator species in that location. 
```{r}
### First, summarize: get mean and sd for virtual predators, bind to prey df; print it
(df.sim <- rbind(prey, data.frame(Taxa = "Simulated_BNT", 
                       m.d13C = round(mean(outdf$C),3), 
                       sd.d13C = round(sd(outdf$C),3), 
                       m.d15N = round(mean(outdf$N),3), 
                       sd.d15N = round(sd(outdf$N),3)) ) )

### Then, summarize for obsereved prey and BNT; print it
trout <- isodat %>%  filter(SpecCode.new == "BNT")  # pull BNT data for plotting
( summ1 <- isodat %>% 
...
### Now visually compare simulated predators to observed predators
...
## 4. Use MixSIAR and SISUS mixing models to estimate the relative contribution of the different prey items to those of your observed predators. 
NOTE: The code below is programed not to run within this R Notebook, so plots and results will not be diplayed below. 
```{r eval=FALSE}
#===============================================================================
### 1. Load data
#===============================================================================
